...
internal sealed class TextureSample : Sample
{
    ...
    protected override void OnInitialize()
    {
        ...
        // Create a descriptor layout with the texture slot
        ...
        // Open a file stream to the texture
        using var stream = File.OpenRead("Texture.png");

        // Load the texture from the stream
        texture = GraphicsDevice.FromStream(stream);

        // Create the descriptor set that will bind the texture
        descriptorSet = new DescriptorSet(GraphicsDevice, descriptorLayout);

        // Bind the texture to the descriptor set slot
        descriptorSet.SetTexture(0, texture);
    }
